...
install -d $RPM_BUILD_ROOT{%{_pkglibdir},/etc/httpd/conf.modules.d}
...
echo %{_sysconfdir}
cp -a %{SOURCE1} $RPM_BUILD_ROOT/etc/httpd/conf.modules.d/90_mod_%{mod_name}.conf
...
%attr(640,root,root) %config(noreplace) %verify(not md5 mtime size) /etc/httpd/conf.modules.d/*_mod_%{mod_name}.conf
